	...
	public int sendEmail(TaskModel taskModel,CaseBean caseBean,LinkedList<UnitLogBean> unitLogs){
		...
		switch(msgSendType){
			...
			case CHECKPOINT_FAIL:
				if(!checkPointFailLog.toString().trim().equals("以下的 CHECKPOINT 执行失败："))msgBean.setMsgContent(commonLog.toString()+checkPointFailLog.toString());
				add(msgBean);
				break;
			case STEP_FAIL:
			if(!stepFailLog.toString().trim().equals("以下的 CHECKPOINT 执行失败："))msgBean.setMsgContent(commonLog.toString()+stepFailLog.toString());
				add(msgBean);
				break;
